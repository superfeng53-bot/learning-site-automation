...
> **Site profile A — 学科规划型**（默认）。公需仅选年份、无学科/申请、**无单日学习上限** → 用 `templates/requirements-year-driven.md`（profile B），见 `site-profiles.md`。  
...
| 打包 | PyInstaller **单文件**；`console=True` 保留终端日志；命名 `{平台中文名}_{月}_{日}`，如 `双卫网_04_22.exe` |
...
- 运营 **重学 / 置顶** 可将 `queued_at` 设为 `now`（或 `0`），故意绕过错峰
...
| 按钮 | 含义 |
|------|------|
| 重学 | 保留 cookies 等登录指纹；清除登录后全部运行数据；下次探活成功则跳过登录，从分配起走完整登录后流程 |
| 编辑重学 | 保存表单变更后执行与「重学」相同的清除与入队 |
...
- [ ] 导入去重、导出列对齐、模板下载
- [ ] 一键启动、单实例、二次启动只开 WebUI、端口避让、打包单文件与 `{平台}_{月}_{日}` 命名
- [ ] **打包后** `python scripts/smoke_frozen.py` exit 0（开发态 `./start.sh` 通过不能代替）
