...
### 一栏凭证模式（`credential_input_mode = combined`）

在 **A 型** 导入列中，于「姓名」之后插入 **「账号密码」** 列（分列「账号」「密码」保留，用于 round-trip 与分列填写）：

| 列序 | 表头 | 必填 | 说明 |
|------|------|------|------|
| A | 姓名 | 否 | 同 §2 |
| B | **账号密码** | 条件必填 | 一栏粘贴；自动识别「账号」「密码」标签或空格/冒号分隔 |
| C | 账号 | 条件必填 | 与密码分列填写时必填；仅填 B 列时可为空 |
| D | 密码 | 条件必填 | 同上 |
| E–K | 学科1…备注 | 否 | 同 §2，列序顺延 |

**导入规则**（`combined` 与 `split` 均适用，只要表头含「账号密码」）：

1. 「账号密码」**有值** → **优先**调用 `credential_parser.parse_combined_credentials` 解析（支持登录名/手机号/工号、帐号/帐户、是/为/等号/括号、【标签】、多行、`|` 四段式等）；解析失败且分列均已填时再回落分列。
2. 否则「账号」「密码」**均有值** → 以分列为准。
3. 三者皆空 → 跳过空行。
4. 仅填其一 → 中文报错（如「账号和密码须同时填写，或改填「账号密码」一栏」）。

**模板示例行（`combined` 必守）**：示例行的「账号」「密码」分列须**留空**，只在「账号密码」列填虚构凭证。禁止 combined 模板同时在分列留示例占位符——否则用户只改「账号密码」列上传时，旧分列值会被误读（历史 bug：`resolve_credentials_from_row` 曾分列优先）。

**导出**：`combined` 项目前 N 列含「账号密码」；该列写入 `账号 {username} 密码 {password}` 以便再次导入（「密码」分列仍默认留空掩码）。后半系统列（状态/错误日志…）导入时忽略。

若 `docs/API_REQUIREMENTS.md` 未选择学科/学分相关需求，可将 D–G 视为可空备注型需求字段，不得因为缺少学科/学分而导入失败。若未选择 `购卡 / 充值`，H/I 必须可空且不触发充值逻辑。

### 填写说明 Sheet 2（A 型固定段落）
...
## 2B. 导入列定义 — Site profile B（公需年度型）

当 `docs/API_REQUIREMENTS.md` 标明 **B — 公需年度型** 时，**替换** §2 的 A–J 学科列形态（勿与 A 型混在同一模板）。参考 `liangshangongxu`：`IMPORT_COLUMNS`。

| 列序 | 表头 | 必填 | 类型 | 说明 |
|------|------|------|------|------|
| A | 账号 | 条件必填 | 文本 | 唯一键；可与「账号密码」二选一 |
| B | 密码 | 条件必填 | 文本 | 导出留空或掩码 |
| C | 备注 | 否 | 文本 | 运营备注 |
| D | 目标年度 | 否 | 文本 | 多选用 `、` 或 `,，;` 分隔；表头别名：`年度` / `年份` / `target_years` |
| E | 任务模式 | 否 | 文本 | `标准`/`normal` 或 `快速`/`fast`；别名：`上报模式` / `report_mode` |

**一栏凭证模式（`combined`）**：在 A 列前插入 **「账号密码」**，原 A–E 顺延为 B–F；导入/导出规则同 §2「一栏凭证模式」。

- 模板文件名仍可为 `{平台中文名}账号模板.xlsx`。
- 至少选一个目标年度；未填时导入默认 **仅当前自然年**（与 Web UI 默认勾选当前年一致）。
- **无** 学科1/学分1 列；导出追加列见下表（可与 A 型后半 K–Q 类似，但 K 起字段名按 B 型需求文档）。

**B 型导出追加列（E 列之后）建议顺序**：

`姓名 | 身份证 | 状态 | 说明 | 重试次数 | 创建时间 | 更新时间 | 最近运行结果 | 错误日志`

（姓名/身份证来自登录后 `extra`，非导入必填。）

### 填写说明 Sheet 2（B 型固定段落）

至少包含：

1. 表头不可改字、不可调列顺序  
2. 账号、密码必填  
3. 目标年度可多选，用顿号或逗号分隔；不填则默认当前年  
4. 任务模式：`标准` 或 `快速`（可选，默认标准）；**若站点 `FAST_REPORT_SUPPORTED=False`，说明 Sheet 写「本站仅支持标准」；导入 `快速` 按标准处理**
5. 示例一行（虚构数据）  

---

## 2B′. 导入列定义 — Site profile B′（项目驱动型）

当 `docs/API_REQUIREMENTS.md` 标明 **B_prime — 项目驱动型** 时，**替换** §2 的 A–J 学科列形态。参考 `医学24/yixue24_service/web/excel_io.py`。
...
| B | 密码 | 条件必填 | 文本 | 导出留空或掩码 |
| C | 备注 | 否 | 文本 | 运营备注 |
| D | 任务模式 | 否 | 文本 | `标准`/`normal` 或 `快速`/`fast`；别名：`上报模式` / `report_mode` |

**一栏凭证模式（`combined`）**：在 A 列前插入 **「账号密码」**；规则同 §2。
...
1. 表头不可改字、不可调列顺序  
2. 账号、密码必填  
3. 任务模式：`标准` 或 `快速`（可选，默认标准）；**若站点不支持快速，说明 Sheet 写「本站仅支持标准」；导入 `快速` 按标准处理**
4. 示例一行（虚构数据）  
...
**前半 — 与导入一致（列数、表头、顺序与当前 `credential_input_mode` 的模板相同）**

- `split`（A 型）：`姓名 | 账号 | 密码 | 学科1 | … | 备注`
- `combined`（A 型）：`姓名 | 账号密码 | 账号 | 密码 | 学科1 | … | 备注`
...
- [ ] **`combined` 模板示例行**：「账号」「密码」分列留空；只填「账号密码」列上传能正确解析（分列无 stale 占位符）
